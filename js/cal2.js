...
// // WHERE
// //   d.idDepartamento = ud.idDepartamento AND ud.idUser = 1


// 			url: 'carga_filtros.php',
// 			type: 'post',
// 			beforeSend: function () {
// 			},
	...
	function muestraDia (e, elemento){
		...
		if (a!=undefined) {
			...
			$(elemento).children("div[id^='reserva']").each(function(index, el) {
				...
				}).html("<span style='color: white;position:relative; z-index:1; width:310px;display:flex;'>"+currentDescripcion+"</span>").css({
					"font-weight": "bold"
				});
	...
	*/
	function crearNuevaReserva(e, elemento){
		...
		if (f1>=f2) {
			...
			muestraMensaje("Solo puede guardar reservas de dias superiores o igual a hoy.");
		}	
	}
...
function compruebaUser(){
	...
	*/
	function cargaReservas(){
		...
		var temp = {}
		//alert(__anyoHoy);
		var parametros = {"mes": parseInt($("#selectMesTexto").val())+1, "ano":$("#anyoVisible").val()};
		$.ajax({
			data: parametros,
		...
		if ($(this).attr("dia")!= undefined && $(this).attr("mes")!= undefined && $(this).attr("anyo")!= undefined) {
			...
			for(var i=0; i<reservasDelMes.length ;i++){
				...
				if (parseInt($(this).attr("dia")) == parseInt(dia) && parseInt($(this).attr("mes"))+1 == parseInt(mes) && parseInt($(this).attr("anyo")) == parseInt(ano) && parseInt($("#SelectSala").val()) == parseInt(idSala)) {
		        	...
		        	$(that).append("<div id='reserva"+idReserva+"' idReserva='"+idReserva+"' dia='"+dia+"' mes='"+mes+"' ano='"+ano+"' horaMin='"+horaMin+"' horaMax='"+horaMax+"' descripcion='"+descripcion+"' userColor='"+color+"' nombre='"+nombre+"' apellido1='"+apellido1+"' apellido2='"+apellido2+"' sala='"+idSala+"' class='reserva draggable' style='float:left; height:30px; width:65px; border-radius:25px; cursor:pointer; font-size: 8pt;background-color: "+color+"; color:white; margin-top: 2px;'><span style='position:relative; top:8px; left:"+izq+"px'>"+ letras +" <span style='font-size:9px; width:30px; display:flex; position:relative; top:-18px; left: 27px;'> "+horaMin.substring(0,5)+" "+horaMax.substring(0,5)+"</span></span></div>");
		        	$("#reserva"+idReserva).bind('dblclick, click', function(event) {
	...
	*/
	function verReserva(elemento){
		...
		var f1 = new Date(c,parseInt(b)-1,a);
		var f2 = new Date();
		f1.setHours(0,0,0,0);
		f2.setHours(0,0,0,0);
		if (nombreUser == (h+" "+i+" "+j) && (f1 >= f2)) {
